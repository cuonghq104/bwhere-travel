...
public class LocationDAOImp implements LocationDAO{
    ...
    public ResponseModel getLocationListByUser(String userId) {
        ...
        String sql = "SELECT travel_place.id, travel_place.name, travel_place.icon, travel_place.zoom, travel_category.icon, travel_category.name_vi, travel_place.latitude, travel_place.longitude, travel_category.id, travel_place.verifyAdmin\n" +
                "FROM travel_place\n" +
                "LEFT JOIN travel_category ON travel_category.id = travel_place.category " +
                "WHERE travel_place.create_by = ?";
        ...
        try {
            ...
            if (rs.last()) {
                ...
                while (rs.next()) {
                    ...
                    double longitude = rs.getDouble(8);
                    String locationTypeId = rs.getString(9);
                    String verifyAdmin = rs.getString(10);
                    //
                    ...
                    location.setLatitude(latitude);
                    location.setLongitude(longitude);
                    location.setStatus((verifyAdmin != null) ? "verified" : "pending");
                    //
